...
import {FloorConstraint} from '@/lib/floor';
const NAMES=['Base rotation','Shoulder','Elbow','Wrist pitch','Wrist yaw','Wrist roll'];
type Motion={from:number[];fromGrip:number;targets:Pose[];index:number;elapsed:number;seconds:number;completion:string};
...
export default function Simulator(){
 ...
 const floor=useMemo(()=>new FloorConstraint(),[]);
 ...
 const start=(targets:Pose[],isSequence=false)=>{
  if(!ready||error||!targets.length)return;
  const c=current.current,route:Pose[]=[];let from={q:c.q,grip:c.grip},completion='Motion complete.';
  for(const target of targets){
   const allowed=floor.limited(from,target);route.push({...target,...allowed});
   if(allowed.q.some((v,i)=>v!==target.q[i])||allowed.grip!==target.grip){completion='Stopped at base plane. Move away to continue.';break}
   from=allowed;
  }
  motion.current={from:c.q.slice(),fromGrip:c.grip,targets:route,index:0,elapsed:0,seconds:duration(c.q,route[0].q,c.grip,route[0].grip),completion};
  isPaused.current=false;setPaused(false);setRunning(true);setActiveIndex(isSequence?0:-1);setMessage(isSequence?'Playing motion sequence.':'Moving to '+targets[0].name.toLowerCase()+'.');
 };
 useEffect(()=>{
 ...
 const applyManual=(requested:{q:number[];grip:number},label:string)=>{
  stop();const accepted=floor.limited(current.current,requested);current.current={...current.current,...accepted};setQ(accepted.q);setGrip(accepted.grip);
  setMessage(accepted.q.some((v,i)=>v!==requested.q[i])||accepted.grip!==requested.grip?'Stopped at base plane. Move away to continue.':label+' adjusted.');setIkMessage('');
 };
 const change=(i:number,v:number)=>{if(Number.isFinite(v))applyManual({q:clampPose(current.current.q.map((n,j)=>i===j?v:n)),grip:current.current.grip},NAMES[i])};
 const changeGrip=(v:number)=>{if(Number.isFinite(v))applyManual({q:current.current.q,grip:Math.max(0,Math.min(90,v))},'Gripper')};
 ...
 return <main className="app-shell">
  ...
  <section className="workspace" aria-label="Robot simulator">
   <div className="viewport"><RobotScene options={{q,grip,grid,axes,trace,view,viewKey}} onReady={()=>setReady(true)} onError={s=>{setError(s);stop()}}/>
    ...
    <div className="camera-views">{['Orbit','Front','Top','Gripper'].map(v=><button key={v} aria-pressed={view===v} className={view===v?'selected':''} onClick={()=>setCamera(v)}>{v}</button>)}</div>
  ...
  <footer className="statusline"><output className="status"><i className="dot"/>{error?'Model unavailable':!ready?'Loading model':message}</output><span>Joint limits on · Solid base plane · No self-collision or forces<br/><a href="/model/NOTICE.txt" target="_blank" rel="noreferrer">Seeed model · CERN-OHL-W-2.0</a></span></footer>
 </main>
}
