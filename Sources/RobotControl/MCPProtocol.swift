...
public final class MCPProtocol {
    ...
    public func handle(_ data: Data) -> [String: Any]? {
        ...
        if method == "initialize" {
            ...
            initialized = true
            return result(id!, ["protocolVersion": version, "serverInfo": ["name": "rebot-motion-lab-codex", "version": ControlCatalog.version], "capabilities": ["tools": ["listChanged": false], "resources": ["subscribe": false, "listChanged": false]], "instructions": "Controls the native B601-DM simulator. Optional cube experiments use RealityKit contact physics; get_task/get_evaluation are privileged diagnostics, while get_observation supplies declared policy inputs. Read state and limits first. Motion tools return acceptance, not physical completion; poll rebot_get_state until playback is stopped and manual_motion is false. Stop existing motion before commanding another pose. Navigating away from Simulator pauses playback and stops manual adjustment. No robot hardware or motor writes."])
        }
