...
public final class MCPProtocol {
    ...
    public func handle(_ data: Data) -> [String: Any]? {
        ...
        if method == "initialize" {
            ...
            initialized = true
            return result(id!, ["protocolVersion": version, "serverInfo": ["name": "rebot-motion-lab-grok", "version": ControlCatalog.version], "capabilities": ["tools": ["listChanged": false], "resources": ["subscribe": false, "listChanged": false]], "instructions": "Controls the native B601-DM kinematic simulator only, including one scene cube and kinematic grasp. Read state and limits first. Scripted motion tools return acceptance, not completion; poll rebot_get_state until playback is stopped and manual_motion is false. In servo mode use rebot_servo_joints / rebot_servo_tcp for immediate updates. Stop existing motion before another scripted pose. No robot hardware, dynamics, or fly-brain model."])
        }
